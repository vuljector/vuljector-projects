...
def _parse_basictest(text: str) -> dict:
    """Ruby basictest: section summaries like '...OK 372' plus a final 'test succeeded'."""
    passed = _sum(r"\bOK\s+(\d+)\b", text)
    failed = 0 if "test succeeded" in text and "test failed" not in text else _sum(r"\bF\b", text) + text.lower().count("not ok")
    return {"passed": passed, "failed": failed}
